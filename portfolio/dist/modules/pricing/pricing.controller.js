...
export const createPricing = async (req, res, next) => {
    try {
        const pricing = await PService.SCreatePricing(req.body);
        if (!pricing) {
...
export const getPricing = async (req, res, next) => {
    try {
        const pricing = await PService.SGetPricing(req.params.id);
        if (!pricing) {
...
export const deletePricing = async (req, res, next) => {
    try {
        const pricing = await PService.SDeletePricing(req.params.id);
        if (!pricing) {
            return next(createHttpError(404, "Pricing not found"));
        }
